#include <stdio.h>
#include <stdlib.h>
double page_fault_rate();//declaring a function which would be called for calculations
void userInput(void);//declaring a function to take user inputs giiven in the problem
//declaring required entities
...
void main(){
	...
	printf("2.Exit\n");
	scanf("%d",&swtch);
	//creating switch for using or exiting the problem calculator
	switch(swtch){
...
//input taken through console
/*Formula:         EAT = (1-p)*(mac) + (p)*(100 + (1-r)*(spfe) + (r)*(spf))   
where EAT=Effective Access Time, p=page fault ,r=(percentage of time)/100,  
spfe=service page fault(not modified), spf=service page fault(modified), mac=memory access time.
*/

	service_page_fault_empty_ns = (service_page_fault_empty*1000000);//converted into absolute value
	service_page_fault_modified_ns = (service_page_fault_modified*1000000);//converted into absolute value
